    ...
    if(mysqli_num_rows($result) > 0) {
      while($row = mysqli_fetch_array($result)) {
        ...
        while($rowmax = mysqli_fetch_array($resmax)) {
          ...
          $output .='
          <form action="add-flight-inserts.php" method="post">
            <input type="hidden" id="dlog_id_set" name="dlog_id_set" value="'.$row["ID"].'">
            <input type="hidden" id="dlog_date_set" name="dlog_date_set" value="'.$select_date_conv.'">
            ...
            <tr style="background-color:#337ab7; color: #fff">
              ...
              if ($row["Is_Positive"] == "+") {$output .= '<td align="right">Hours to Next Check: <br><strong>'.$row["Hours_To_Next_Check"];}
                else {$output .= "<td align='right' style='background-color: yellow; color: red;'>Hours to Next Check: <br><strong>". $row["Hours_To_Next_Check"];};
                $output .= '</strong></td>
            </tr>
            <tr>
              <td>Flight No</td>
            ...
            <tr>
              <td>Uplift Oil</td>
              <td><input tabindex="11" type="text" name="Uplift_Oil" placeholder="Liters" id="Uplift_Oil" value="0" required></td>
            ...
            <tr>
              <td>Defects</td>
              <td colspan=5><input tabindex="16" name="Defects" value="NIL" required size="50"></td>
            </tr>
          </table>
    ...
    else
    {
      ...
      <form action="add-flight-inserts.php" method="post">
        ...
        <tr style="background-color:#337ab7; color: #fff">
          ...
          <td>Enginer Start-Up</td>
          <td><input tabindex="6" type="time" name="Engine_Start_Up" id="Engine_Start_Up" required></td>
        </tr>
        <tr>
          <td>Captain</td>
          <td colspan=3><input tabindex="2" type="text" name="Captain" id="Captain" value="'.$_SESSION["username"].'" required></td>
          <td>Engine Shutdown</td>
          <td><input tabindex="7" type="time" name="Engine_Shutdown" id="Engine_Shutdown" required</td>
        </tr>
        <tr>
          <td>P2 / Passenger</td>
          <td colspan=3><input tabindex="3" type="text" name="P2_Passenger" id="P2_Passenger" required></td>
          <td>Take Off Time</td>
          <td><input tabindex="8" type="time" name="Takeoff_Time" id="Takeoff_Time" required></td>
        </tr>
        <tr>
          <td>From</td>
          <td colspan=3><input tabindex="4" type="text" name="From_Airport" id="From_Airport" value="Gloucester EGBJ" required></td>
          <td>Landing Time</td>
          <td><input tabindex="9" type="time" name="Landing_Time" id="Landing_Time" required></td>
        </tr>
        <tr>
          <td>To</td>
          <td colspan=3><input tabindex="5" type="text" name="To_Airport" id="To_Airport" value="Gloucester EGBJ" required></td>
          <td>Landings</td>
          <td><input tabindex="10" type="number" name="Landings" id="Landings" required></td>
        </tr>
        <tr style="background-color:#337ab7; color: #fff">
          <td colspan=6">Fuel / Oil Log</td>
        </tr>
        <tr>
          <td>Uplift Fuel</td>
          <td><input tabindex="11" type="text" name="Uplift_Fuel" placeholder="Liters" id="Uplift_Fuel" required></td>
          <td>Departure Fuel</td>
          <td><input tabindex="13" type="text" name="Departure_Fuel" placeholder="Liters" id="Departure_Fuel" required></td>
          <td>Arrival Fuel</td>
          <td><input tabindex="15" type="text" name="Arrival_Fuel" placeholder="Liters" id="Arrival_Fuel" required></td>
        </tr>
        <tr>
          <td>Uplift Oil</td>
          <td><input tabindex="12" type="text" name="Uplift_Oil" placeholder="Liters" id="Uplift_Oil" value="0" required></td>
          <td>Departure Oil OK?</td>
